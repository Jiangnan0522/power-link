...
from power_link.baselines import HeteroGNNExplainer
...
from power_link.utils import print_args, set_seed
...
if args.without_path_loss:
    FILENAME = f'gnnexp_explain_results_{p.encoder}_{p.score_func}_{p.dataset}_unpath' 
elif not args.pagelink_loss:
    FILENAME = f'gnnexp_explain_results_{p.encoder}_{p.score_func}_{p.dataset}'
else:
    FILENAME = f'gnnexp_explain_results_{p.encoder}_{p.score_func}_{p.dataset}_pl'
