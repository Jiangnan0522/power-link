...
class RelGraphConv(nn.Module):
    ...
    def forward(self, g, feat, etypes, norm=None, eweight=None, message_passing=True):
        """Forward computation.
        ...
        with g.local_scope():
            ...
            if not self.wni:
                ...
                if message_passing:
                    ...
                    msg_func = lambda edges, etypes: {k: th.zeros_like(v) for k, v in self.message_func(edges, etypes).items()}

                g.update_all(functools.partial(msg_func, etypes=etypes),
