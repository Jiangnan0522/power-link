...
class CompGCNCov(nn.Module):
    def __init__(self, in_channels, out_channels, act=lambda x: x, bias=True, drop_rate=0., opn='corr', num_base=-1,
        ...
        if num_rel is not None:
            self.rel = self.get_param([num_rel*2, in_channels])
        else:
            self.rel = None
        self.opn = opn
    ...
    def message_func(self, edges):
        ...
        if '_edge_weight' in edges.data:
            if isinstance(edges.data['_edge_weight'], dict):
                mask = list(edges.data['_edge_weight'].values())[0] # * We only have one type of nodes and edges in the graph for KG
            else:
    ...
    def forward(self, g: dgl.DGLGraph, x, rel_repr, edge_type, edge_norm, eweight=None, message_passing=True):
        """
        ...
        if message_passing:
            ...
            msg_func = lambda edges: {k: torch.zeros_like(v) for k, v in self.message_func(edges).items()}

        g.update_all(msg_func, fn.sum(
